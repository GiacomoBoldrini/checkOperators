...
#  switchOn = ['21']
  ...
  if len(sys.argv) < 2:
     print("Usage: python create_table.py <proc_id>")
     sys.exit(0)
  ...
  results = glob("{}_*_QU.tar.xz".format(proc_ID))
  available_ops = [i.split("{}_".format(proc_ID))[1].split("_")[0] for i in results] 
  
  indices = [int(i[0]) for i in params]
  shape = (int(mt.floor(mt.sqrt(128))), int(mt.ceil(mt.sqrt(128))))
  ...
  for i in range(int(shape[0])):
    for j in range(int(shape[1])):
       if idx >= len(params): continue
       op = params[idx][1]
       if op in available_ops: matrix[i,j]=1
       idx += 1
  ...
  plt.imshow(matrix, cmap=cm.RdYlGn)
  idx = 0
  for i in range(int(shape[0])):
    for j in range(int(shape[1])):
       if idx >= len(params): continue
       plt.text(j, i, params[idx][1] , verticalalignment="center", horizontalalignment="center", color="white", fontsize=9)
       idx += 1
